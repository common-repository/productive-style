...
function productive_style_register_blocks_init_action_team_member() {
    productive_style_register_block_init_team_member();
}
add_action( 'init', 'productive_style_register_blocks_init_action_team_member' );


function productive_style_register_block_init_team_member() {
    
    global $productive_style_plugin_version;
    
    $asset_file = include( PRODUCTIVE_STYLE_PLUGIN_PATH . 'includes/common/gutenberg/blocks/team-member/build/index.asset.php');
    
    wp_register_script(
        'productive-style-team-member-script',
        PRODUCTIVE_STYLE_PLUGIN_URI . 'includes/common/gutenberg/blocks/team-member/build/index.js',
        $asset_file['dependencies'],
        $asset_file['version']
    );
    
    wp_register_style(
        'productive-style-team-member-style',
        PRODUCTIVE_STYLE_PLUGIN_URI . 'includes/common/gutenberg/blocks/team-member/build/style-index.css',
        array(),
        $productive_style_plugin_version
    );
    
    $block_metadata = PRODUCTIVE_STYLE_PLUGIN_PATH . 'includes/common/gutenberg/blocks/team-member/build';
    $args = array(
        'api_version' => 3,
        'version' => $productive_style_plugin_version,
        'render_callback' => 'productive_style_register_block_render_callback_team_member',
        'editor_script' => 'productive-style-team-member-script',
        'style' => 'productive-style-team-member-style',
    );
    register_block_type( $block_metadata, $args );
}
function productive_style_register_block_render_callback_team_member( $attributes, $content ) {
    ...
    $section_content_items_css_classes = ' ' . $section_content_content_box_shape . ' ' . $section_content_content_box_alignment_h . ' ' . $section_content_show_url_button_shape;
    ...
    do_action( 'display_content_wrapper_full_full_top', $section_bg_color_scheme );
        do_action( 'display_content_wrapper_full_top', $section_content_items_css_classes );
            productive_style_render_team( $cpt_section_args );
