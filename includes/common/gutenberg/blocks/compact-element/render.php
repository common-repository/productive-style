...
function productive_style_register_blocks_init_action_compact_element() {
    productive_style_register_block_init_compact_element();
}
add_action( 'init', 'productive_style_register_blocks_init_action_compact_element' );


function productive_style_register_block_init_compact_element() {
    
    global $productive_style_plugin_version;
    
    $asset_file = include( PRODUCTIVE_STYLE_PLUGIN_PATH . 'includes/common/gutenberg/blocks/compact-element/build/index.asset.php');
    
    wp_register_script(
        'productive-style-compact-element-script',
        PRODUCTIVE_STYLE_PLUGIN_URI . 'includes/common/gutenberg/blocks/compact-element/build/index.js',
        $asset_file['dependencies'],
        $asset_file['version']
    );
    
    wp_register_style(
        'productive-style-compact-element-style',
        PRODUCTIVE_STYLE_PLUGIN_URI . 'includes/common/gutenberg/blocks/compact-element/build/style-index.css',
        array(),
        $productive_style_plugin_version
    );
    
    $block_metadata = PRODUCTIVE_STYLE_PLUGIN_PATH . 'includes/common/gutenberg/blocks/compact-element/build';
    $args = array(
        'api_version' => 3,
        'version' => $productive_style_plugin_version,
        'render_callback' => 'productive_style_register_block_render_callback_compact_element',
        'editor_script' => 'productive-style-compact-element-script',
        'style' => 'productive-style-compact-element-style',
    );
    register_block_type( $block_metadata, $args );
}
function productive_style_register_block_render_callback_compact_element( $attributes, $content ) {
    ...
    $section_bg_color_scheme        = 'productive-paddable-section page_main_section_container ' . $section_gtbg_align . ' ' . $attributes['section_bg_color_scheme'] . ' ' . $attributes['section_block_max_width'];
    
    ob_start();
    
    do_action( 'display_content_wrapper_full_full_top', $section_bg_color_scheme );
        do_action('display_content_wrapper_full_top');
            productive_style_render_compact_element( $cpt_section_args );
