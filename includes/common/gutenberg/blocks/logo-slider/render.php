...
function productive_style_register_blocks_init_action_logo_slider() {
    productive_style_register_block_init_logo_slider();
}
add_action( 'init', 'productive_style_register_blocks_init_action_logo_slider' );


function productive_style_register_block_init_logo_slider() {
    
    global $productive_style_plugin_version;
    
    $asset_file = include( PRODUCTIVE_STYLE_PLUGIN_PATH . 'includes/common/gutenberg/blocks/logo-slider/build/index.asset.php');
    
    wp_register_script(
        'productive-style-logo-slider-script',
        PRODUCTIVE_STYLE_PLUGIN_URI . 'includes/common/gutenberg/blocks/logo-slider/build/index.js',
        $asset_file['dependencies'],
        $asset_file['version']
    );
    
    wp_register_style(
        'productive-style-logo-slider-style',
        PRODUCTIVE_STYLE_PLUGIN_URI . 'includes/common/gutenberg/blocks/logo-slider/build/style-index.css',
        array(),
        $productive_style_plugin_version
    );
    
    $block_metadata = PRODUCTIVE_STYLE_PLUGIN_PATH . 'includes/common/gutenberg/blocks/logo-slider/build';
    $args = array(
        'api_version' => 3,
        'version' => $productive_style_plugin_version,
        'render_callback' => 'productive_style_register_block_render_callback_logo_slider',
        'editor_script' => 'productive-style-logo-slider-script',
        'style' => 'productive-style-logo-slider-style',
    );
    register_block_type( $block_metadata, $args );
}
function productive_style_register_block_render_callback_logo_slider( $attributes, $content ) {
    ...
    do_action( 'display_content_wrapper_full_full_top', $section_bg_color_scheme );
        do_action('display_content_wrapper_full_top');
            productive_style_render_logo_slider( $cpt_section_args );
